...
${profileBlock(profile, roleFamily, level)}
...
export function gradePrompts({ profile, roleFamily, level, materialsText, question, category, transcript, durationSec, wpm, fillers }) {
  const materials = materialsText
    ...
    : '';
  return {
    system: `You are a rigorous, kind Google interview coach grading one spoken answer. You grade against the bar for ${level} (${level === 'L6' ? 'leads through others, multi-team scope' : 'senior IC, large-project ownership'}). Be honest — a 7 means "solid but not yet there", reserve 9+ for genuinely excellent. Scores must be earned by the sub-scores.
